...
class PublicationData:
    ...
    def publish_artifacts(self, content, prefix=""):
        ...
        package_builds_by_nevra: dict[str, list[PkgBuild]] = defaultdict(list)
        package_builds_by_path: dict[str, list[PkgBuild]] = defaultdict(list)
        cid_to_pkginfo: dict[UUID, PackageInfo] = {}
        cid_to_2nd_path: dict[UUID, str] = {}
        cids_to_ignore: set[UUID] = set()
        ...
        for row in contentartifact_qs.values(*fields).iterator():
            ...
            pkg_build = PkgBuild(cid=cid, build_time=build_time)

            # First, get the nevra and add it to our uniquifying dict
            nevra = format_nevra(
                row["content__rpm_package__name"],
                row["content__rpm_package__epoch"],
                row["content__rpm_package__version"],
                row["content__rpm_package__release"],
                row["content__rpm_package__arch"],
            )
            package_builds_by_nevra[nevra].append(pkg_build)
            ...
            if layout == LAYOUT_TYPES.NESTED_ALPHABETICALLY:
                pkg_path = index_path = nested_alphabetically_path(pkg_filename)
            elif layout == LAYOUT_TYPES.FLAT:
                pkg_path = index_path = flat_path(pkg_filename)
            elif layout == LAYOUT_TYPES.NESTED_BY_DIGEST:
                pkg_path = index_path = nested_by_digest_path(pkg_filename, checksum)
            elif layout == LAYOUT_TYPES.NESTED_BY_BOTH:
                # We still don't want to publish duplicates that collide on the alphabetical path,
                # but we want the metadata to reference the by_digest path.
                pkg_path = nested_by_digest_path(pkg_filename, checksum)
                index_path = nested_alphabetically_path(pkg_filename)
                cid_to_2nd_path[cid] = index_path
            else:
                raise ValueError("Layout value is unsupported by this version")

            # Store info indexed by path
            pkg_info = PackageInfo(
                caid=caid, path=pkg_path, checksum_type=checksum_type, checksum=checksum
            )
            package_builds_by_path[index_path].append(pkg_build)
            cid_to_pkginfo[cid] = pkg_info

        # Ignore all but the "newest" package that collide on nevra
        for nevra, build_list in package_builds_by_nevra.items():
            if len(build_list) > 1:
                build_list.sort(key=lambda x: x.build_time, reverse=True)
                cids_to_ignore.update(x.cid for x in build_list[1:])
                log.warning(
                    ...
                    "the most recent build time, excluding {others} others.".format(
                        nevra=nevra, others=len(build_list[1:])
                    )
                )

        # Ignore all but the "newest" package that collide on path
        for rel_path, build_list in package_builds_by_path.items():
            if len(build_list) > 1:
                # First, if a cid is already ignored because of nevra, ignore it here too
                build_list = [x for x in build_list if x.cid not in cids_to_ignore]
                if len(build_list) > 1:
                    build_list.sort(key=lambda x: x.build_time, reverse=True)
                    cids_to_ignore.update(x.cid for x in build_list[1:])
                    log.warning(
        ...
        for cid, pkg_info in cid_to_pkginfo.items():
            ...
            if layout == LAYOUT_TYPES.NESTED_BY_BOTH:
                ...
                published_artifacts.append(
                    PublishedArtifact(
                        relative_path=os.path.join(prefix, cid_to_2nd_path[cid]),
                        publication=self.publication,
                        content_artifact_id=pkg_info.caid,
                    )
                )
...
def generate_repo_metadata(
    ...
    Args:
        ...
        layout(pulp_rpm.app.constants.LAYOUT_TYPES):
            How to layout the package files within the publication (flat, nested, etc.)
        packages(dict):
            A dictionary of packages included in the publication.
